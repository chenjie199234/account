...
export class UserBrowserClientToC {
	...
	update_oauth(header: { [k: string]: string },req: UpdateOauthReq,timeout: number,errorf: (arg: Error)=>void,successf: (arg: UpdateOauthResp)=>void){
		...
		let body: string=''
		try{
			body=UpdateOauthReqToJson(req)
		}catch(e){
			errorf({code:-2,msg:''+e})
			return
		}
		let config={
			url:_WebPathUserUpdateOauth,
			method: "post",
			baseURL: this.host,
			headers: header,
			data: body,
		...
		.then(function(response){
			let obj:UpdateOauthResp
			try{
				obj=JsonToUpdateOauthResp(response.data)
			}catch(e){
				let err:Error={code:-1,msg:'response body decode failed'}
				errorf(err)
				return
	...
	del_oauth(header: { [k: string]: string },req: DelOauthReq,timeout: number,errorf: (arg: Error)=>void,successf: (arg: DelOauthResp)=>void){
		...
		let body: string=''
		try{
			body=DelOauthReqToJson(req)
		}catch(e){
			errorf({code:-2,msg:''+e})
			return
		}
		let config={
			url:_WebPathUserDelOauth,
			method: "post",
			baseURL: this.host,
			headers: header,
			data: body,
		...
		.then(function(response){
			let obj:DelOauthResp
			try{
				obj=JsonToDelOauthResp(response.data)
			}catch(e){
				let err:Error={code:-1,msg:'response body decode failed'}
				errorf(err)
				return
