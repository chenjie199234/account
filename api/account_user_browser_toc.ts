...
function UpdateIdcardReqToJson(msg: UpdateIdcardReq): string{
	...
	//old_receiver_type
	if(msg.old_receiver_type==null||msg.old_receiver_type==undefined){
		throw 'UpdateIdcardReq.old_receiver_type must be string'
	}else{
		//transfer the json escape
		let vv=JSON.stringify(msg.old_receiver_type)
		s+='"old_receiver_type":'+vv+','
	...
	//old_dynamic_password
	if(msg.old_dynamic_password==null||msg.old_dynamic_password==undefined){
		throw 'UpdateIdcardReq.old_dynamic_password must be string'
	}else{
		//transfer the json escape
		let vv=JSON.stringify(msg.old_dynamic_password)
		s+='"old_dynamic_password":'+vv+','
...
function UpdateNickNameReqToJson(msg: UpdateNickNameReq): string{
	...
	//old_receiver_type
	if(msg.old_receiver_type==null||msg.old_receiver_type==undefined){
		throw 'UpdateNickNameReq.old_receiver_type must be string'
	}else{
		//transfer the json escape
		let vv=JSON.stringify(msg.old_receiver_type)
		s+='"old_receiver_type":'+vv+','
	...
	//old_dynamic_password
	if(msg.old_dynamic_password==null||msg.old_dynamic_password==undefined){
		throw 'UpdateNickNameReq.old_dynamic_password must be string'
	}else{
		//transfer the json escape
		let vv=JSON.stringify(msg.old_dynamic_password)
		s+='"old_dynamic_password":'+vv+','
...
export class UserBrowserClientToC {
	...
	nick_name_duplicate_check(header: { [k: string]: string },req: NickNameDuplicateCheckReq,timeout: number,errorf: (arg: Error)=>void,successf: (arg: NickNameDuplicateCheckResp)=>void){
		...
		let body: string=''
		try{
			body=NickNameDuplicateCheckReqToJson(req)
		}catch(e){
			errorf({code:-2,msg:''+e})
			return
		}
		let config={
			url:_WebPathUserNickNameDuplicateCheck,
			method: "post",
			baseURL: this.host,
			headers: header,
			data: body,
	...
	idcard_duplicate_check(header: { [k: string]: string },req: IdcardDuplicateCheckReq,timeout: number,errorf: (arg: Error)=>void,successf: (arg: IdcardDuplicateCheckResp)=>void){
		...
		let body: string=''
		try{
			body=IdcardDuplicateCheckReqToJson(req)
		}catch(e){
			errorf({code:-2,msg:''+e})
			return
		}
		let config={
			url:_WebPathUserIdcardDuplicateCheck,
			method: "post",
			baseURL: this.host,
			headers: header,
			data: body,
	...
	email_duplicate_check(header: { [k: string]: string },req: EmailDuplicateCheckReq,timeout: number,errorf: (arg: Error)=>void,successf: (arg: EmailDuplicateCheckResp)=>void){
		...
		let body: string=''
		try{
			body=EmailDuplicateCheckReqToJson(req)
		}catch(e){
			errorf({code:-2,msg:''+e})
			return
		}
		let config={
			url:_WebPathUserEmailDuplicateCheck,
			method: "post",
			baseURL: this.host,
			headers: header,
			data: body,
	...
	tel_duplicate_check(header: { [k: string]: string },req: TelDuplicateCheckReq,timeout: number,errorf: (arg: Error)=>void,successf: (arg: TelDuplicateCheckResp)=>void){
		...
		let body: string=''
		try{
			body=TelDuplicateCheckReqToJson(req)
		}catch(e){
			errorf({code:-2,msg:''+e})
			return
		}
		let config={
			url:_WebPathUserTelDuplicateCheck,
			method: "post",
			baseURL: this.host,
			headers: header,
			data: body,
