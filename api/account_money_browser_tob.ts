...
export class RechargeMoneyReq{
	toJSON(){
		let tmp = {}
		return tmp
	}
}
export class RechargeMoneyResp{
	fromOBJ(_obj:Object){
	}
}
export class RefundMoneyReq{
	toJSON(){
		let tmp = {}
		return tmp
	}
}
export class RefundMoneyResp{
	fromOBJ(_obj:Object){
	}
}
...
export class SpendMoneyReq{
	toJSON(){
		let tmp = {}
		return tmp
	}
}
export class SpendMoneyResp{
	fromOBJ(_obj:Object){
	}
}
...
export class MoneyBrowserClientToB {
	...
	//timeout's unit is millisecond,it will be used when > 0
	recharge_money(header: Object,req: RechargeMoneyReq,timeout: number,error: (arg: LogicError)=>void,success: (arg: RechargeMoneyResp)=>void){
		if(!header){
			header={}
		}
		header["Content-Type"] = "application/json"
		...
		call(timeout,this.host+"/admin.app/proxy",{method:"POST",headers:header,body:JSON.stringify(realreq)},error,function(arg: Object){
			let r=new RechargeMoneyResp()
			r.fromOBJ(arg)
			success(r)
		})
	}
	//timeout's unit is millisecond,it will be used when > 0
	spend_money(header: Object,req: SpendMoneyReq,timeout: number,error: (arg: LogicError)=>void,success: (arg: SpendMoneyResp)=>void){
		if(!header){
			header={}
		}
		header["Content-Type"] = "application/json"
		...
		call(timeout,this.host+"/admin.app/proxy",{method:"POST",headers:header,body:JSON.stringify(realreq)},error,function(arg: Object){
			let r=new SpendMoneyResp()
			r.fromOBJ(arg)
			success(r)
		})
	}
	//timeout's unit is millisecond,it will be used when > 0
	refund_money(header: Object,req: RefundMoneyReq,timeout: number,error: (arg: LogicError)=>void,success: (arg: RefundMoneyResp)=>void){
		if(!header){
			header={}
		}
		header["Content-Type"] = "application/json"
