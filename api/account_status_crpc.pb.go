...
func (c *statusCrpcClient) Ping(ctx context.Context, req *Pingreq) (*Pingresp, error) {
	...
	if errstr := req.Validate(); errstr != "" {
		slog.ErrorContext(ctx, "[/account.Status/Ping] request validate failed", slog.String("error", errstr))
		return nil, cerror.ErrReq
	}
	...
	if e := c.cc.Call(ctx, _CrpcPathStatusPing, reqd, crpc.Encoder_PROTOBUF, func(ctx *crpc.CallContext) error {
		...
		if respbody, encoder, err = ctx.Recv(); err != nil {
			slog.ErrorContext(ctx, "[/account.Status/Ping] read response failed", slog.String("error", err.Error()))
		}
		return err
	}); e != nil {
		if !execute {
			slog.ErrorContext(ctx, "[/account.Status/Ping] send request failed", slog.String("error", e.Error()))
		}
		return nil, e
	}
	resp := new(Pingresp)
	...
	case crpc.Encoder_PROTOBUF:
		if e := proto.Unmarshal(respbody, resp); e != nil {
			slog.ErrorContext(ctx, "[/account.Status/Ping] decode response failed", slog.String("error", e.Error()))
	...
	default:
		slog.ErrorContext(ctx, "[/account.Status/Ping] unknown response encoder")
		return nil, cerror.ErrResp
	}
	return resp, nil
}
...
func _Status_Ping_CrpcHandler(handler func(context.Context, *Pingreq) (*Pingresp, error)) crpc.OutsideHandler {
	return func(ctx *crpc.ServerContext) {
		...
		if e != nil {
			slog.ErrorContext(ctx, "[/account.Status/Ping] read request failed", slog.String("error", e.Error()))
			ctx.Abort(e)
			return
		}
		req := new(Pingreq)
		...
		case crpc.Encoder_PROTOBUF:
			if e := proto.Unmarshal(reqbody, req); e != nil {
				slog.ErrorContext(ctx, "[/account.Status/Ping] request decode failed", slog.String("error", e.Error()))
		...
		default:
			slog.ErrorContext(ctx, "[/account.Status/Ping] request encoder unknown")
			ctx.Abort(cerror.ErrReq)
			return
		}
		if errstr := req.Validate(); errstr != "" {
		...
		if e := ctx.Send(respd, encoder); e != nil {
			slog.ErrorContext(ctx, "[/account.Status/Ping] send response failed", slog.String("error", e.Error()))
		}
	}
}
