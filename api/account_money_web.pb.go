...
func _Money_SelfMoneyLogs_WebHandler(handler func(context.Context, *SelfMoneyLogsReq) (*SelfMoneyLogsResp, error)) web.OutsideHandler {
	return func(ctx *web.Context) {
		...
		if strings.HasPrefix(ctx.GetContentType(), "application/json") {
			data, e := ctx.GetBody()
			if e != nil {
				slog.ErrorContext(ctx, "[/account.money/self_money_logs] get body failed", slog.String("error", e.Error()))
				ctx.Abort(e)
				return
			}
			if len(data) > 0 {
				if e := (protojson.UnmarshalOptions{AllowPartial: true, DiscardUnknown: true}).Unmarshal(data, req); e != nil {
					slog.ErrorContext(ctx, "[/account.money/self_money_logs] unmarshal json body failed", slog.String("error", e.Error()))
			...
			if len(data) > 0 {
				if e := proto.Unmarshal(data, req); e != nil {
					slog.ErrorContext(ctx, "[/account.money/self_money_logs] unmarshal proto body failed", slog.String("error", e.Error()))
					ctx.Abort(cerror.ErrReq)
					return
				}
			}
			...
			if e := ctx.ParseForm(); e != nil {
				slog.ErrorContext(ctx, "[/account.money/self_money_logs] parse form failed", slog.String("error", e.Error()))
				ctx.Abort(cerror.ErrReq)
				return
			}
			// req.StartTime
			if form := ctx.GetForm("start_time"); len(form) != 0 {
				if num, e := strconv.ParseUint(form, 10, 32); e != nil {
					slog.ErrorContext(ctx, "[/account.money/self_money_logs] data format wrong", slog.String("field", "start_time"))
					ctx.Abort(cerror.ErrReq)
					return
				} else {
			...
			if form := ctx.GetForm("end_time"); len(form) != 0 {
				if num, e := strconv.ParseUint(form, 10, 32); e != nil {
					slog.ErrorContext(ctx, "[/account.money/self_money_logs] data format wrong", slog.String("field", "end_time"))
					ctx.Abort(cerror.ErrReq)
					return
				} else {
			...
			if form := ctx.GetForm("page"); len(form) != 0 {
				if num, e := strconv.ParseUint(form, 10, 32); e != nil {
					slog.ErrorContext(ctx, "[/account.money/self_money_logs] data format wrong", slog.String("field", "page"))
					ctx.Abort(cerror.ErrReq)
					return
				} else {
					req.Page = uint32(num)
		...
		if errstr := req.Validate(); errstr != "" {
			slog.ErrorContext(ctx, "[/account.money/self_money_logs] validate failed", slog.String("error", errstr))
			ctx.Abort(cerror.ErrReq)
			return
		}
